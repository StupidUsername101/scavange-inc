class_name FourLimbPPOUpdateJob
extends RefCounted

#######################################################
# Runs one detached four-limb PPO update on a private worker thread. The live trainer and
...
func run() -> Dictionary:
	...
	if not (config_value is Dictionary):
		return _failure("The four-limb optimizer received no valid configuration.")
	if not (rollout_value is Array) or (rollout_value as Array).is_empty():
		return _failure("The four-limb optimizer received no rollout samples.")
	if not (network_value is Dictionary):
		return _failure("The four-limb optimizer received no network state.")

	var worker = FourLimbPPOTrainer.new(int(payload.get("random_seed", 7340033)))
	...
	if (
		...
		return _failure("The four-limb optimizer network state was incompatible.")
	...
	for transition_value: Variant in (rollout_value as Array):
		if transition_value is Dictionary:
			# The live trainer transfers ownership of this rollout before the thread starts. The
			# optimizer may hydrate deferred critic values in these private transition dictionaries;
			# no live physics-side code retains them after begin_background_update().
			...
			if int(transition.get("policy_revision", expected_policy_revision)) != expected_policy_revision:
				return _failure("The four-limb optimizer received a mixed-policy rollout.")
			worker.rollout.append(transition)
	if worker.rollout.is_empty():
		return _failure("The four-limb optimizer received no valid rollout transitions.")
	...
	if metrics.is_empty() or metrics.has("error"):
		return _failure(str(metrics.get(
			"error",
			worker.last_error if not worker.last_error.is_empty() else "The four-limb PPO update failed."
		)))
	return {
		"ok": true,
