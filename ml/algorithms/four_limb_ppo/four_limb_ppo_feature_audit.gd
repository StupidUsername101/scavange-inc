class_name FourLimbPPOFeatureAudit
extends RefCounted
...
#######################################################
# Audits the actual normalized four-limb PPO rollout. Exact empirical dependence cannot be established
...
static func status_text(report: Dictionary) -> String:
	if int(report.get("sample_count", 0)) < 2:
		return "Limb PPO is waiting for enough movement data to check its body inputs."
