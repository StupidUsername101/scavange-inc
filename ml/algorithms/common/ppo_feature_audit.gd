...
const MAXIMUM_REPORTED_CORRELATION_PAIRS: int = 16

#######################################################
# Shared low-cadence PPO feature diagnostics. The analyzer works on the normalized actor inputs
# recorded in rollouts and reports constants, strong Pearson correlation and the numerical rank of
# centered/standardized samples without allocating a full feature-by-feature rank matrix.
#######################################################
...
static func analyze_samples(
	...
	var valid_samples: Array[PackedFloat64Array] = []
	for raw_sample: Variant in samples:
		if not (raw_sample is PackedFloat64Array):
			continue
		var sample: PackedFloat64Array = raw_sample
		if sample.size() != feature_count or not RLTrainingMath.packed_all_finite(sample):
			continue
		valid_samples.append(sample)
	if valid_samples.size() < 2:
		empty_report["sample_count"] = valid_samples.size()
		empty_report["maximum_observable_rank"] = maxi(
			mini(valid_samples.size() - 1, feature_count),
			0
		)
		return empty_report
	sample_count = valid_samples.size()

	var means: PackedFloat64Array = PackedFloat64Array()
	means.resize(feature_count)
	means.fill(0.0)
	for sample: PackedFloat64Array in valid_samples:
	...
	for row_position: int in range(varying_indices.size()):
		...
		for column_position: int in range(row_position + 1, varying_indices.size()):
			...
			max_absolute_correlation = maxf(max_absolute_correlation, absolute_correlation)
...
static func status_text(
	...
	if int(report.get("sample_count", 0)) < 2:
		return "%s is waiting for enough %s to check its %s." % [
			trainer_label,
			data_label,
			input_label,
		]
	var high_pairs: Array = report.get("high_correlation_pairs", [])
	var high_pair_count: int = int(report.get("high_correlation_pair_count", high_pairs.size()))
...
static func _record_strong_pair(
	...
	var weakest_strength: float = absf(float(pairs[0].get("correlation", 0.0)))
	for pair_index: int in range(1, pairs.size()):
		var strength: float = absf(float(pairs[pair_index].get("correlation", 0.0)))
...
static func _standardized_sample_rank(
	samples: Array[PackedFloat64Array],
	...
	var maximum_rank: int = mini(varying_indices.size(), samples.size() - 1)
	for sample: PackedFloat64Array in samples:
		var vector: PackedFloat64Array = PackedFloat64Array()
		vector.resize(varying_indices.size())
		var original_norm_squared: float = 0.0
		...
		var residual_norm_squared: float = 0.0
		...
		if (
			...
			continue
		var inverse_norm: float = 1.0 / sqrt(residual_norm_squared)
		for position: int in range(vector.size()):
