class_name TurretPPOActorCritic
extends RefCounted

const STATE_SCHEMA_VERSION = 5
const ACTION_COUNT = TurretMLAction.ACTION_COUNT
...
func sample_action(observation: Dictionary, deterministic: bool = false) -> Dictionary:
	return _sample_action(observation, deterministic, true)


func sample_runtime_action(observation: Dictionary, deterministic: bool = false) -> Dictionary:
	return _sample_action(observation, deterministic, false)
...
func _sample_action(
	...
	var input = TurretMLFeatureEncoder.encode(observation)
	return sample_action_from_input(
		observation,
		input,
		deterministic,
		include_action_dictionary
	)
...
func sample_action_from_input(
	...
	if not TurretMLFeatureEncoder.is_normalized(input):
		return {}
	var mean = actor.predict_reusable(input)
	var value_output = critic.predict_reusable(input)
...
func deterministic_action(observation: Dictionary) -> Dictionary:
	var sample = sample_action(observation, true)
...
func load_state(state: Dictionary) -> bool:
	...
	if (
		...
		return false
	var staged: TurretPPOActorCritic = TurretPPOActorCritic.new(
		7340033,
		loaded_hidden_size,
...
func _load_state_in_place(state: Dictionary) -> bool:
	if (
		...
		or str(state.get("body_profile_id", "")) != TurretPhysicalBody3D.BODY_PROFILE_ID
		or RLTrainingMath.finite_int_or(state.get("observation_schema_version", 0), -1) != TurretMLObservation.SCHEMA_VERSION
		or RLTrainingMath.finite_int_or(state.get("action_schema_version", 0), -1) != TurretMLAction.SCHEMA_VERSION
		or RLTrainingMath.finite_int_or(state.get("action_count", 0), -1) != ACTION_COUNT
		or RLTrainingMath.finite_int_or(state.get("feature_count", 0), -1) != TurretMLFeatureEncoder.FEATURE_COUNT
...
func copy_from(source: TurretPPOActorCritic) -> bool:
