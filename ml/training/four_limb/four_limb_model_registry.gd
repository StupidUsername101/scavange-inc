...
const DEFAULT_ROOT_PATH = "user://ml_four_limb_models"
...
#######################################################
# Separate model library for four-limb bodies. Drone and limb checkpoints cannot be selected
...
func save_checkpoint(model_name: String, checkpoint: Dictionary) -> Dictionary:
	...
	if not _is_compatible_checkpoint(checkpoint):
		last_error = "The checkpoint is not a compatible four-limb model."
		return {}
	var clean_name = model_name.strip_edges() if not model_name.strip_edges().is_empty() else "Four Limb Model"
	...
	if not TrainingFileIO.preserve_next_version_floor(
		...
		last_error = "Could not preserve the four-limb model version sequence."
...
func overwrite_checkpoint(record_or_id: Variant, checkpoint: Dictionary) -> Dictionary:
	last_error = ""
	if not _is_compatible_checkpoint(checkpoint):
		last_error = "The replacement checkpoint is not a compatible four-limb model."
		return {}
	var record = _resolve(record_or_id)
	if record.is_empty():
		last_error = "The rolling four-limb model version no longer exists."
		return {}
	if (
		...
		last_error = "The rolling version belongs to a different limb body or learning algorithm."
		return {}
	var version_path = str(record.get("storage_path", ""))
	if version_path.is_empty():
		last_error = "The rolling four-limb model has no storage directory."
		return {}
	...
	if not _write_json(checkpoint_path, checkpoint):
		last_error = "Could not overwrite the rolling four-limb checkpoint."
		return {}
	...
	if not _write_json(version_path.path_join(MANIFEST_FILE_NAME), record):
		if _write_json(checkpoint_path, previous_checkpoint):
			last_error = "Could not update the rolling four-limb model manifest; the previous checkpoint was restored."
		else:
...
func display_name(record: Dictionary) -> String:
	return "%s %s" % [str(record.get("model_name", "Four Limb Model")), str(record.get("version_name", "v????"))]
...
func _is_compatible_checkpoint(checkpoint: Dictionary) -> bool:
	var metadata_matches: bool = (
		...
		and str(checkpoint.get("artifact_type", "")) == "trained_four_limb_policy"
		...
		and RLTrainingMath.finite_int_or(checkpoint.get("action_schema_version", 0), -1) == FourLimbMLAction.SCHEMA_VERSION
		and RLTrainingMath.finite_int_or(checkpoint.get("action_count", 0), -1) == FourLimbMLAction.ACTION_COUNT
...
func _key(value: String) -> String:
	return TrainingFileIO.storage_key(value, "four-limb-model")
