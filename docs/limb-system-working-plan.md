...
### Pass 49 live-editable Core geometry invariant
- Creator drone Cores own serialized polygon topology through `DroneCoreEditableMeshDefinition`: explicit vertices plus logical polygon faces. Rendering triangulates those polygons into an `ArrayMesh`, but authored topology remains separate so later edge/face subdivision does not have to reconstruct quads or adjacency from render triangles.
- The topology exposes unique logical edges independently of render-triangle diagonals. Pass 49 only exposes face selection plus face-area scaling in the UI; edge selection/subdivision is intentionally a later editing operation on the same persisted topology rather than a different mesh format.
- Creator Core dimensions X/Y/Z are live-editable. Resizing updates persisted geometry, preview mesh, physical bounds, and reprojects already-authored mount transforms to the edited convex surface instead of leaving mounts at obsolete box coordinates.
- Face Edit mode changes LMB from mount placement to polygon picking. The selected polygon is highlighted and may be expanded or shrunk around its own center; the preview rebuilds immediately while preserving camera orbit/zoom. Leaving Face Edit restores mount placement.
- Runtime/training visual geometry uses the authored Core mesh rather than an implicit `BoxMesh`. Dynamic Core collision uses one `ConvexPolygonShape3D` from the authored vertices; custom Core meshes therefore remain compatible with a moving `RigidBody3D` while avoiding concave/trimesh collision on dynamic workers.
- The old hard-coded decorative X arms in the training-room drone visual are removed. A training worker now displays the actual authored Core and real mounted hardware, so custom/spider geometry is not visually overwritten by legacy quad-drone decoration.
- The generic resource snapshot/checkpoint path must preserve the nested editable Core topology. Existing Cores without topology lazily materialize a six-face box from their saved `body_size`, preserving old resource defaults while making them editable on first use.
- Godot is not installed in this workspace; the pass includes authored integration regressions and static/resource validation, but the live editor/physics behavior must be exercised in the user's Godot 4.6 runtime.

### Pass 51 stability-first cleanup / creator-flight invariant
- `DroneTrainingRoom` remains the orchestration owner for simulation, spawning, episodes, group state, and training. Plot-series assembly and drone action-trace normalization are pure helpers (`DroneTrainingPlotSeriesBuilder` and `DroneTrainingActionCodec`) so presentation/telemetry growth does not keep expanding the room controller. The room retains a small compatibility delegate for the existing plot-series test/debug seam.
- Runtime and training/loadout diagnostics use one induced-power implementation in `DroneRotorPhysics`. The inverse power equation has one fixed denominator floor shared by both callers; valid minimum-radius/minimum-efficiency creator rotors therefore cannot be rated with one power equation and flown with another.
- The linked per-rotor power editor reserves non-camera attachment idle draw in the battery/core bus ceilings, matching `ServerDrone`'s runtime allocation order where attachment idle power is consumed before rotor power. A selected linked rotor power must remain physically available after installed attachment overhead.
- Editable Core topology validation fails closed on malformed persisted face offsets, out-of-range vertex indices, non-monotonic spans, and non-finite vertices before render/collision code can index the topology. Corrupt creator data must not turn `has_geometry()` or `face_indices()` into an out-of-bounds path.
- Shared registry persistence primitives live in `TrainingFileIO`: JSON-dictionary reads and atomic text promotion/rollback no longer have independent copies in drone/map/limb/turret registries and reward-card storage.
- Compact-vs-full drone observation snapshot selection belongs to `ServerDrone`, so training and deterministic evaluation cannot drift on which runtime snapshot a model contract expects.
- The room physics pass caches active-group state once per tick and prunes invalid training items once before recovery/spatial refresh. Public/default helper behavior remains pruning-safe for direct test/debug callers.
- The current four-propeller runtime cap and stock-quad SAC/SAC-HER restriction remain intentional. The larger PPO/SAC/turret/four-limb optimizer/trainer similarities were audited but deliberately not merged in this stability pass because those algorithm cores have a much higher regression blast radius without an executable Godot regression run.

### Pass 52 body-invariant / editor-cleanup invariant
- `DroneLoadout` owns Core-slot validity. Propellers, AI chips, and attachments cannot be installed or exposed without a Core, removing a Core clears every Core-mounted part, and Core replacement asks the loadout whether occupied slots would be lost. Creator/edit-session code must not reimplement those array scans.
- Core slot counts from persisted/custom resources are treated as untrusted structure input. Negative counts clamp to zero before range/index operations so malformed resources fail closed instead of addressing from an array tail.
- Training loadout duplication and limb-action accounting iterate only slots actually supported by the current Core; orphan/raw backing-array entries must not leak into a copied worker body or its ML contract.
- Training-room editor raycasts are centralized in `DroneTrainingEditorRaycast`. Item placement, destination placement, authored-item selection, and custom-wall selection share one retry/exclusion implementation while preserving their distinct authored-surface policies.
- Plot catalog metadata belongs with `DroneTrainingPlotSeriesBuilder`, reward-component display names belong with `DroneTrainingRoomPresentation`, and registry filesystem/naming primitives belong in `TrainingFileIO`; the room/controller should not accumulate presentation catalogs or repeated persistence utilities.
- Four-limb and turret model registries preserve a monotonic next-version floor just like the drone library. Deleting the newest saved version must never allow a later checkpoint to reuse that immutable `version_id`; gaps after failed/deleted saves are preferable to identity reuse.
- Descriptor-less PPO recovery recognizes the historical propeller fallback only when the entire action width is the four-control legacy quad contract. Wider creator bodies with missing descriptors recover neutral generic controls rather than silently assigning rotor semantics/startup bias to their first four channels. Normal accepted bodies retain explicit descriptors and do not use this fallback.
- The current four-propeller `ServerDrone`/training/evaluation cap remains intentional and is now stated explicitly in `ml/README.md`; variable snapshot/body-manifest topology does not by itself mean the runtime already supports more than four physical rotors.
- Large exact overlaps between the drone/turret/four-limb PPO/SAC trainer implementations remain known architectural debt. They are intentionally not merged in this stability pass because optimizer/update behavior has a much larger regression surface without an executable Godot regression suite.
